...
uint8_t MasterStation::getParent(PMessage p) {
    ...
    for (int i = 0; i < 5; i++) {
        if (canTalkTo[nextId][i] != 0) {
            ...
            if (nFreePipe > 0) {
                uint16_t temp = (4 - nFreePipe) + getLevel(canTalkTo[nextId][i]);
                if (weight > temp) {
...
void MasterStation::receivedAskConfig(PMessage p) {
    ...
    if (p.id_dest == id || GET_MSG_DEST(p.proto) == 1) {
        ...
        r = PMessage(PMessage::PROTOCOL, PMessage::SET_CONFIG, (uint8_t) 0x00,
                (uint8_t) id, nextId, (uint8_t) (getLevel(parent) + 1), parent);
